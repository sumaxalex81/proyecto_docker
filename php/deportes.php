...
<div class="contenido_publicaciones">
    <div class="foronoticias">
        ...
        <?php if (isset($_SESSION['usuario'])): ?>
            <button class="boton_publicar" type="button" onclick="toggleFormDep()">Publicar</button>
        <?php else: ?>
    ...
    <form id="formPublicacion" class="form_publicacion" action="publicar.php" method="post" enctype="multipart/form-data" style="display:none;">
        ...
        <div class="layout-vertical">
            <textarea name="contenido" rows="4" placeholder="Escribe tu publicación sobre deportes..." required></textarea>
        ...
        if ($resultado && $resultado->num_rows > 0) {
            while ($fila = $resultado->fetch_assoc()) {
                ...
                if (isset($_SESSION['usuario']) && $_SESSION['usuario'] === 'Admin') {
                    echo "<form action='eliminar_publicacion_admin.php' method='post' class='form-eliminar'><input type='hidden' name='id' value='$idPub'><input type='hidden' name='pagina' value='deportes.php'><button type='submit' class='btn-eliminar'>Eliminar</button></form>";
                }
                echo "<div class='contenedor-boton-responder'><a href='comentarios.php?id=$idPub' class='btn-responder'>Responder</a></div>";
                echo "</div><hr>";
            }
        } else {
            echo "<p style='color:white;'>No hay publicaciones en Deportes.</p>";
        }
        ?>
    </div>
</div>
